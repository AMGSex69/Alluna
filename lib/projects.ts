...
export async function createProject(projectData: {
	...
	try {
		const { data, error } = await supabaseModule.supabase.from("projects").insert([projectData]).select().single()

		if (error) {
			if (error.message.includes("Could not find the table")) {
				console.warn("Projects table not found. Please run the database setup script.")
				return null
			}
...
export async function getProjects(): Promise<Project[]> {
	...
	try {
		const { data, error } = await supabaseModule.supabase.from("projects").select("*").order("created_at", { ascending: false })
...
export async function getProject(id: string): Promise<Project | null> {
	...
	try {
		const { data, error } = await supabaseModule.supabase.from("projects").select("*").eq("id", id).single()
...
export async function deleteProject(id: string): Promise<boolean> {
	...
	try {
		...
		const { error: documentsError } = await supabaseModule.supabase.from("documents").delete().eq("project_id", id)
		...
		const { error } = await supabaseModule.supabase.from("projects").delete().eq("id", id)
...
export async function createDocument(documentData: {
	...
	try {
		...
		const { data, error } = await supabaseModule.supabase.from("documents").insert([documentData]).select().single()
...
export async function getDocument(id: string): Promise<Document | null> {
	...
	try {
		const { data, error } = await supabaseModule.supabase.from("documents").select("*").eq("id", id).single()
...
export async function deleteDocument(id: string): Promise<boolean> {
	...
	try {
		const { error } = await supabaseModule.supabase.from("documents").delete().eq("id", id)
