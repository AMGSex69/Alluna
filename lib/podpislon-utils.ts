...
export async function generateDocumentPDF(data: PDFData): Promise<string> {
  try {
    ...
    // Используем только simple PDF генератор
    const pdf = generateSimplePDF(data)
...
export function formatPhoneForPodpislon(phone: string): string {
  ...
  throw new Error(`Invalid phone format: ${phone}. Expected formats: +7 XXX XXX-XX-XX, 8 XXX XXX-XX-XX, etc.`)
}
