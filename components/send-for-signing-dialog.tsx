...
import { Send, Mail, Phone, AlertCircle, User, FileText, Download } from "lucide-react"
...
export function SendForSigningDialog({
	...
	const handleGeneratePDF = async () => {
		...
		try {
			...
			if(!pdfData || (!pdfData.startsWith('data:application/pdf;base64,') && !pdfData.startsWith('dataL'))) {
			...
			setError(error instanceof Error ? error.message : "Ошибка при генерации PDF документа")
		} finally {
	...
	const handleDownloadPDF = () => {
		if (pdfPreview) {
			...
			link.download = `Договор_${documentName}_${new Date().toISOString().split('T')[0]}.pdf`
			link.click()
		}
	}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!pdfPreview.startsWith('data:') && !pdfPreview.startsWith('data:application/pdf')) {
			setError("Сгенерированный PDF имеет неверный формат. Попробуйте сгенерировать ещё раз.")
			return
		}
		...
		try {
			...
			setError(error instanceof Error ? error.message : "Произошла ошибка при отправке документа")
		} finally {
			setIsLoading(false)
		}
	}
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="sm:max-w-[600px]">
				<DialogHeader>
					...
					<DialogDescription>
						Документ "{documentName}" будет автоматически сгенерирован и отправлен клиенту на подписание.
					</DialogDescription>
				</DialogHeader>
					...
					<div className="grid gap-4 py-4">
						...
						<div className="grid gap-2">
							...
							<div className="relative">
								...
								<Input
									...
									className={`pl-10 ${!validateName(name) && name ? 'border-orange-500' : ''}`}
									required
								/>
							</div>
						...
						<div className="grid gap-2">
							...
							<div className="relative">
								...
								<Input
									...
									className={`pl-10 ${!validatePhone(phone) && phone ? 'border-orange-500' : ''}`}
									required
								/>
							</div>
							<p className="text-xs text-muted-foreground">
						...
						<div className="border rounded-lg p-4 bg-gray-50">
							<div className="flex items-center justify-between mb-3">
								...
								<Button
									...
									disabled={isGeneratingPDF || !name || !validateName(name) || !phone || !validatePhone(phone) || !email || !validateEmail(email)}
								>
									{isGeneratingPDF ? (
										<>
								...
								<div className="space-y-3">
									<div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded">
										<div className="flex items-center gap-2">
											...
											<span className="text-green-800 font-medium">PDF документ готов</span>
										</div>
										<Button
											type="button"
											variant="outline"
							...
							{!pdfPreview && (
								<p className="text-sm text-muted-foreground">
									Нажмите "Сгенерировать PDF" чтобы создать документ для подписания
								</p>
							)}
						</div>
						...
						<div className="rounded-lg border border-gray-200 p-3 bg-gray-50">
							...
							<ol className="text-sm text-muted-foreground space-y-1">
								...
								<li>3. На email клиента придет письмо со ссылкой для подписания</li>
