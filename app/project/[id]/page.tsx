...
import { ArrowLeft, FileText, Mail, Phone, Clock, CheckCircle, Loader2 } from "lucide-react"
...
import { getProject, getProjectDocuments, createDocument, updateDocumentStatus } from "@/lib/projects"
...
export default function ProjectPage() {
	...
	useEffect(() => {
		const loadProjectData = async () => {
			...
			try {
				const [projectData, documentsData] = await Promise.all([getProject(projectId), getProjectDocuments(projectId)])
	...
	const handleSendForSigning = async (documentId: string, phone: string, email?: string) => {
		console.log("[v0] Sending document for signing via Email:", documentId, { phone, email })
		...
		try {
			...
			if (response.ok) {
				...
				const success = await updateDocumentStatus(documentId, "pending_signature")

				if (success) {
					setDocuments((prev) =>
						prev.map((doc) => (doc.id === documentId ? { ...doc, status: "pending_signature" as const } : doc)),
					)

					console.log(`[v0] Email sent to ${email}: Ссылка для подписания отправлена`)
		...
		return (
			<div className="min-h-screen bg-gray-50 flex items-center justify-center">
				<div className="text-center">
					<h2 className="text-xl font-semibold text-gray-900 mb-2">{error || "Проект не найден"}</h2>
		...
		<div className="min-h-screen bg-gray-50">
			...
			<main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
					...
					<Card className="mb-6">
						...
						<CardContent>
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div>
									<p className="text-sm font-medium text-gray-600">Имя клиента</p>
				...
				{signedDocuments.length > 0 && (
					<div className="mb-8">
						<h3 className="text-lg font-semibold text-gray-900 mb-4">Быстрый доступ к подписанным документам</h3>
										...
										<div className="flex items-center gap-3">
											...
											<div className="flex-1">
												...
												<p className="text-xs text-gray-600">
													Подписан {document.signed_at ? new Date(document.signed_at).toLocaleDateString("ru-RU") : ""}
												</p>
											</div>
										</div>
									</CardContent>
				...
				<div className="mb-8">
					<div className="flex justify-between items-center mb-6">
						<div>
							<h3 className="text-xl font-semibold text-gray-900">Документы проекта</h3>
							<p className="text-sm text-gray-600 mt-1">Договоры, приложения и акты по этапам</p>
						</div>
										...
										<div className="flex items-center gap-4">
											...
											<div>
												...
												<p className="text-sm text-gray-600">
													...
													{document.signed_at && (
														<> • Подписан {new Date(document.signed_at).toLocaleDateString("ru-RU")}</>
													)}
												</p>
											</div>
										</div>
										<div className="flex items-center gap-3">
											...
											{document.status === "draft" && (
												<SendForSigningDialog
													...
													onSendForSigning={(phone, email) => handleSendForSigning(document.id, phone, email)}
												/>
											)}
											...
											<Button variant="outline" size="sm" onClick={() => handleDocumentClick(document.id)}>
												Просмотр
											</Button>
										</div>
									</div>
					...
					{documents.length === 0 && (
						<div className="text-center py-12">
							...
							<h3 className="text-lg font-medium text-gray-900 mb-2">Пока нет документов</h3>
							<p className="text-gray-600 mb-6">Создайте первый документ для этого проекта</p>
						</div>
					)}
				</div>
			</main>
