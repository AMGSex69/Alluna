...
export async function POST(request: NextRequest) {
  try {
    ...
    const apiKey =
      process.env.OKIDOKI_API_KEY || "67gZSOPuU6ahC5h3ZFTlICsjj1sBuMW-";
    const fullUrl = baseUrl.startsWith("http") ? baseUrl : `https://${baseUrl}`;

    if (!apiKey) {
      return NextResponse.json(
        { error: "OkiDoki API key is not configured" },
        { status: 500 }
      );
    }
    ...
    const contractBody = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #ecfdf5; padding: 15px; border-radius: 8px; margin: 20px 0;">
          ...
          <p>Дата создания: <strong>${new Date().toLocaleDateString(
            "ru-RU"
          )}</strong></p>
        </div>
    ...
    const payload = {
      ...
      system_entities: [
        {
          keyword: "client_first_name",
          value: signer.name.split(" ")[0] || "",
        },
        ...
        { keyword: "client_email", value: signer.email },
        {
          ...
          value: `Договор для подписания: ${document_name}`,
        },
      ],
    ...
    console.log(
      "[OkiDoki] Sending request to:",
      `${fullUrl}/external/contract`
    );
